...
# Extra packages
RUN apt-get update && apt-mark hold keyboard-configuration && apt-get install git expect qemu-system-x86 xfce4 dbus-x11 -y
...
RUN echo xfce4-session >~/.xsession \
    && sed -i 's/3389/${PORT}/g' /etc/xrdp/xrdp.ini  # Menyesuaikan port dinamis Railway
...
# Start XRDP and XFCE4 on container startup
CMD rm /railway/Dockerfile && service xrdp start && startxfce4 && python3 ngrok_.py
